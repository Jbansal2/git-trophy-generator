...
describe('Helpers - sanitizeParams', () => {
  ...
  it('should clamp column between 1-10', () => {
    assert.strictEqual(sanitizeParams({ username: 'test', column: '0' }).column, 6);
    assert.strictEqual(sanitizeParams({ username: 'test', column: '15' }).column, 10);
  });
});
...
describe('Helpers - formatErrorMessage', () => {
  it('should format known errors', () => {
    ...
    assert.strictEqual(message, 'GitHub user not found. Please check the username.');
  });
